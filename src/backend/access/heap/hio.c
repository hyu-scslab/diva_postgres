 ...
 *
 * Place a tuple with a dummy tuple at specified page
 */
void
RelationPutHeapTupleWithDummy(Relation relation,
		Buffer buffer,
		HeapTuple tuple,
		bool token)
{
 ...
 *
 * Wrapper function of RelationGetBufferForTuples.
 * Just pass the parameter num_tuples with 1.
 */
Buffer
...
{
	  return RelationGetBufferForTuples(relation, len, otherBuffer, options,
				                    bistate, vmbuffer, vmbuffer_other, 1);
}

/*
 * RelationGetBufferForTuples
 *
 ...
 */
Buffer
RelationGetBufferForTuples(Relation relation, Size len,
		Buffer otherBuffer, int options,
		BulkInsertState bistate,
		Buffer *vmbuffer, Buffer *vmbuffer_other,
		int num_tuples)
{
	...
	if (num_tuples == 1)
		len = MAXALIGN(len) * num_tuples;   /* be conservative */
	else
	{
		len = MAXALIGN(len) * num_tuples + MAXALIGN(PITEM_SZ);
		num_tuples = 3;
	}
	...
	if (targetBlock == InvalidBlockNumber && use_fsm)
	{
		...
		targetBlock = GetPageWithFreeSpace(relation,
				len + (sizeof(ItemIdData) * (num_tuples - 1)) + saveFreeSpace);
...
loop:
	while (targetBlock != InvalidBlockNumber)
	{
		...
		/* Get free space of the page except the size for line pointers */
		pageFreeSpace = PageGetHeapFreeSpaceWithLP(page, num_tuples);
		 ...
		 * to try.
		 */
		pageFreeSpace += sizeof(ItemIdData) * (num_tuples - 1);
		targetBlock = RecordAndGetPageWithFreeSpace(
				relation, targetBlock, pageFreeSpace,
				len + (sizeof(ItemIdData) * (num_tuples - 1)) + saveFreeSpace);
	}
	...
	if (needLock)
	{
		...
		else if (!ConditionalLockRelationForExtension(relation, ExclusiveLock))
		{
			 ...
			 */
			targetBlock = GetPageWithFreeSpace(relation,
					len + (sizeof(ItemIdData) * (num_tuples - 1)) +
					saveFreeSpace);
	...
	if (otherBuffer != InvalidBuffer)
	{
		...
		if (unlikely(!ConditionalLockBuffer(otherBuffer)))
		{
			 ...
			 * from start.
			 */
			if (len > PageGetHeapFreeSpaceWithLP(page, num_tuples))
			{
				LockBuffer(otherBuffer, BUFFER_LOCK_UNLOCK);
				UnlockReleaseBuffer(buffer);
