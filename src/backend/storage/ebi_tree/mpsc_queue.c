 ...
 * mpsc_queue.c
 *
 * MPSC Queue for EBI Tree Implementation
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/storage/ebi_tree/mpsc_queue.c
 *
 *-------------------------------------------------------------------------
 */
#ifdef J3VM
...
dsa_pointer InitQueue(dsa_area* area) {
...
void DeleteQueue(dsa_area* area, dsa_pointer dsa_queue) {
  TaskQueue queue;
...
bool IsEmpty(dsa_area* area, dsa_pointer dsa_queue) {
  TaskQueue queue;
  queue = (TaskQueue)dsa_get_address(area, dsa_queue);
  return queue->tail == queue->head;
}

void Enqueue(dsa_area* area, dsa_pointer dsa_queue, dsa_pointer dsa_node) {
  TaskQueue queue;
  TaskNode tail;
  ...
  while (!success) {
    ...
    success = pg_atomic_compare_exchange_u32((pg_atomic_uint32*)(&tail->next),
                                             &expected, dsa_new_tail);
...
dsa_pointer Dequeue(dsa_area* area, dsa_pointer dsa_queue) {
  TaskQueue queue;
...
void PrintQueue(dsa_area* area, dsa_pointer dsa_queue) {
  TaskQueue queue;
  dsa_pointer dsa_tmp;
  TaskNode tmp;
