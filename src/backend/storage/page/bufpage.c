...
//	bool    check_empty;
	...
	/* Set the line pointer for p-leaf meta slot */
	itemId = PageGetItemId(phdr, offsetNumber + 2);
	ItemIdSetNormal(itemId, upper + (int) alignedSize * 2, PITEM_SZ);
	LP_OVR_SET_UNUSED(itemId);
	LP_SET_PLEAF_FLAG(itemId);
	...
	/* zero the pleaf meta item */
	memset((char *) page + upper + (int) alignedSize * 2, 0x00, (Size) PITEM_SZ);
